...
#include "ClassifierOut/TMVAClassification_125_nJ2_mu_Likelihood.class.C"
#include "ClassifierOut/TMVAClassification_125_VBF_mu_Likelihood.class.C"
#include "ClassifierOut/TMVAClassification_BDT.class_ggh_mu.C"
#include "ClassifierOut/TMVAClassification_BDT.class_vbf_mu.C"
...
void kanamuon::myana(double myflag, bool isQCD, int runflag)
{
   ...
   if ( !isQCD ) {
      ...
      /*if (myflag == 20122600 || myflag == -300){
        InitCounters( inDataDir + "crabmerge_MC_Wmunu_ggH600.root", h_events, h_events_weighted);             
        myChain = new TChain("WJet");  
...
void kanamuon::Loop(TH1F* h_events, TH1F* h_events_weighted, int wda, int runflag, const char *outfilename, bool isQCD)
{
   ...
   float ggh125_ang_ha   = 999, ggh125_ang_hb = 999, ggh125_ang_hs = 999, ggh125_ang_phi = 999, ggh125_ang_phia = 999, ggh125_ang_phib = 999;
   float      ggh125_Mlvjj = 999, ggh125_ptlvjj =-999,  ggh125_ylvjj = -999, ggh125_philvjj = -999, ggh125_wjj_m = 999;

   float       GenMlvjj=999;

   float qqh125_ang_ha   = 999, qqh125_ang_hb = 999, qqh125_ang_hs = 999, qqh125_ang_phi = 999, qqh125_ang_phia = 999, qqh125_ang_phib = 999;
   float      qqh125_Mlvjj = 999, qqh125_ptlvjj =-999,  qqh125_ylvjj = -999, qqh125_philvjj = -999, qqh125_wjj_m = 999;

   float       dPhi_lep_dijet=999;
   float       vbfdPhi_lep_dijet=999;

  Float_t mvacombin_2j125mu[6][6];
  Float_t mvavbf_comb_2j125mu[6][6];

//  Int_t Indexmax_mvaout[2];
   Float_t max_mvaout125mu;
   Float_t max_mvaoutvbf125mu;


   Float_t mva2j125mu = 999, mva2j160mu = 999, mva2j170mu = 999, mva2j180mu = 999, mva2j190mu = 999, mva2j200mu = 999, mva2j250mu = 999, mva2j300mu = 999, mva2j350mu = 999, mva2j400mu = 999, mva2j450mu = 999, mva2j500mu = 999, mva2j550mu = 999, mva2j600mu = 999;
   ...
   TBranch * branch_comb_2j125mu =  newtree->Branch("mvacombin_2j125mu",   &mvacombin_2j125mu,    "mvacombin_2j125mu[15]/F");
   TBranch * branch_max_mvaout125mu = newtree->Branch("max_mvaout125mu",   &max_mvaout125mu,  "max_mvaout125mu/F");
   TBranch * branch_mvavbf_comb_2j125mu   =  newtree->Branch("mvavbf_comb_2j125mu",   &mvavbf_comb_2j125mu,    "mvavbf_comb_2j125mu[15]/F");
   TBranch * branch_max_mvaoutvbf125mu = newtree->Branch("max_mvaoutvbf125mu",   &max_mvaoutvbf125mu,  "max_mvaoutvbf125mu/F");
   ...
   TBranch *branch_qqh125_wjj_m     = newtree->Branch("qqh125_wjj_m",     &qqh125_wjj_m,      "qqh125_wjj_m/F");
   TBranch *branch_ggh125_wjj_m     = newtree->Branch("ggh125_wjj_m",     &ggh125_wjj_m,      "ggh125_wjj_m/F");
...
//   TBranch * branch_2j125mu   =  newtree->Branch("mva2j125mu",   &mva2j125mu,    "mva2j125mu/F");
   ...
   //TBranch * branch_vbf125mu   =  newtree->Branch("mvavbf125mu",   &mvavbf125mu,    "mvavbf125mu/F");
   ...
   ReadMVA2j125mu mvaReader2j125mu( inputVarsMVA );
 ...
 //const std::vector<double>& inputValuesMVA;
   ...
   for (int i=0; i<11; ++i) ggh125_inputVarsMVA.push_back( ggh125_inputVars[i] );
	...
	ReadBDTgghmu mvaReaderggh125mu ( ggh125_inputVarsMVA );
 ...
 //const std::vector<double>& inputValuesMVA;
   ...
   for (int i=0; i<11; ++i) qqh125_inputVarsMVA.push_back( qqh125_inputVars[i] );
        ...
        ReadBDTvbfmu mvaReaderqqh125mu ( qqh125_inputVarsMVA );
...
//        int   count3 =0;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    // mvacombin_2j125mu = 999; 

   mvacombin_2j125mu[6][6]= {-10};
   //Indexmax_mvaout[2] = {-1,-1};
   max_mvaout125mu = 999;

   mvavbf_comb_2j125mu[6][6]= {-10};
   max_mvaoutvbf125mu = 999;


     mva2j125mu =999;  mva2j160mu = 999; mva2j170mu = 999; mva2j180mu = 999; mva2j190mu = 999; mva2j200mu = 999; mva2j250mu = 999; mva2j300mu = 999; mva2j350mu = 999; mva2j400mu = 999; mva2j450mu = 999; mva2j500mu = 999; mva2j550mu = 999; mva2j600mu = 999;
...
*/
		   ...
		   dijet_p = leading_jet_p+nextleading_jet_p;
					...
					dPhi_lep_dijet = signed_delta_phi(Phi_dijet_p, W_muon_phi);
...
//		                       MrecoMet_pt = event_metMVA_met;
  //              		       MrecoMet_phi = event_metMVA_metPhi;
					...
					ggh125_inputVarsMVAvalue.clear();
				        ...
				        mvacombin_2j125mu[l][m] =  mvaReaderggh125mu.GetMvaValue( ggh125_inputVarsMVAvalue );
	       			 float newmax = find_max(greatest, mvacombin_2j125mu[l][m]);
				 greatest = newmax;

				if (greatest == mvacombin_2j125mu[l][m]){
//				if (sqrt(pow(W_Parton_eta[j]-JetPFCor_Eta[k],2)+pow(delta_phij,2))<0.3)
                                //gg125evt = gg125evt+1;
                                max_mvaout125mu=greatest;
				Indexmax_mvaout[0] = l;
				Indexmax_mvaout[1] =m;}

//				        cout<<"event "<<jentry<<"mvaout  "<<mva2j125el[l][m]<<endl;
				}	
			}			
		}
	}
...
if ( W_muon_pt >25. && JetPFCor_Pt[Indexmax_mvaout[0]] >20. && JetPFCor_Pt[Indexmax_mvaout[1]] >20. && event_met_pfmet >20.){
...
gg125evt =1;
			...
			leading_lepton.SetPtEtaPhiE(W_muon_pt, W_muon_eta, W_muon_phi, W_muon_e);
...
//			dijet = leading_jet+nextleading_jet;
                        ggh125_wjj_m = (leading_jet+nextleading_jet).M();
...
//:::::::::::::::::::::::::::::::::::::::::::::::;end:::::::::::::::::::::::::::::::;;
      ...
      nvp.SetPxPyPzE(event_met_pfmet * cos(event_met_pfmetPhi), event_met_pfmet * sin(event_met_pfmetPhi), 
            //nvp.SetPxPyPzE(event_metMVA_met * cos(event_metMVA_metPhi), event_metMVA_met * sin(event_metMVA_metPhi), 
         W_pzNu1, sqrt(event_met_pfmet*event_met_pfmet + W_pzNu1*W_pzNu1)                     );
            // W_pzNu1, sqrt(event_metMVA_met * event_metMVA_met + W_pzNu1*W_pzNu1)                     );
      ...
      if ( !isQCD ) {
         ...
         //if ( !(event_metMVA_met >25.0) ) isgengdboostedWevt = 0; //Move to MVA MET Later
      } else {
         //keep muons with iso>0.20 (loose=0.20;tight=0.12)
         if ( !(muoniso>0.12)         ) {isgengdboostedWevt = 0;}
      }
      ...
      if (isgengdevt) { ggdevt = 4;// Do the kinematic fit for all event!!!
         ...
         //mva2j125mu = (float) mvaReader2j170mu.GetMvaValue( mvaInputVal );
...
*/
                   ...
                   TLorentzVector vbf_dijet_q;
                   vbf_dijet_q = vbf_jet1_p+vbf_jet2_p;

                                        float Phi_vbfdijet_p = vbf_dijet_q.Phi();
                                        vbfdPhi_lep_dijet = signed_delta_phi(Phi_vbfdijet_p, W_muon_phi);
...
//                                     MrecoMet_pt = event_metMVA_met;
  //                                   MrecoMet_phi = event_metMVA_metPhi;
                                        ...
                                        mvavbf_comb_2j125mu[l][m] =  mvaReaderqqh125mu.GetMvaValue( qqh125_inputVarsMVAvalue );
                                 float newmax = find_max(greatest, mvavbf_comb_2j125mu[l][m]);
                                 vbfgreatest = newmax;

                                if (vbfgreatest == mvavbf_comb_2j125mu[l][m]){
//                              if (sqrt(pow(W_Parton_eta[j]-JetPFCor_Eta[k],2)+pow(delta_phij,2))<0.3)
                                //gg125evt = gg125evt+1;
                                max_mvaoutvbf125mu=vbfgreatest;
                                Indexmax_mvaoutvbf[0] = l;
                                Indexmax_mvaoutvbf[1] =m;}

//                                      cout<<"event "<<jentry<<"mvaout  "<<mvavbf2j125el[l][m]<<endl;
                                }
                        }
                }
        }

max_mvaoutvbf.push_back(greatest);

if (Indexmax_mvaoutvbf[0] != -1 && Indexmax_mvaoutvbf[1] !=-1){
if ( W_muon_pt >25. && JetPFCor_Pt[Indexmax_mvaoutvbf[0]] >20. && JetPFCor_Pt[Indexmax_mvaoutvbf[1]] >20. && event_met_pfmet >20.){
//cout<<" "<<jentry<<" greatest "<<max_mvaout.at(jentry)<<"   "<<Indexmax_mvaout[0]<<"  "<<Indexmax_mvaout[1]<<" "<<MatchedRecoJetIndex[0]<<" "<<MatchedRecoJetIndex[1]<<"    "<<Index[0]<<"   "<<Index[1]<<endl;
vbfcount1 = vbfcount1+1;
qq125evt =1;
                        TLorentzVector vbf_lepton;
                        vbf_lepton.SetPtEtaPhiE(W_muon_pt, W_muon_eta, W_muon_phi, W_muon_e);
...
//                      TLorentzVector dijet;
  //                    dijet = vbf1_jet+vbf2_jet;
                     qqh125_wjj_m = (vbf1_jet+vbf2_jet).M();
...
//                      v4.SetPtEtaPhiM(event_metMVA_met, 0., event_metMVA_metPhi, 0.);
           		...
           		qqh125_ptlvjj     = (vbf_lepton+vbfv4+vbf1_jet+wjj_bjp).Pt();
...
//	branchmet_pt->Fill();
//	branchmet_phi->Fill();
//	branchdijetlep_dphi->Fill();
        branch_max_mvaout125mu->Fill();
        branch_max_mvaoutvbf125mu->Fill();
        //branch_index125el->Fill();
//        branch_2j125mu->Fill();

        branch_comb_2j125mu->Fill();
        branch_mvavbf_comb_2j125mu->Fill();
...
float kanamuon::find_max ( float a, float b)
{ if (a > b) return a;
else return b;
}


		float kanamuon::signed_delta_phi(float phi1, float phi2)
		{
