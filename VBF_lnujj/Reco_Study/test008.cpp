...
//  TBranch        *b_GroomedJet_CA[6]_tau2tau1;   //!
  ...
  WJet->SetBranchAddress ("W_H_e_gen",    &W_H_e_gen,    &b_W_H_e_gen   ) ;
   ...
   TH1F h_dparton1_pt ("h_dparton1_pt", "h_dparton1_pt", 100, 0, 500) ;
/*   TH1F h_dparton1_eta ("h_dparton1_eta", "h_dparton1_eta", 100, -5, 5) ;
   TH1F h_dparton2_pt ("h_dparton2_pt", "h_dparton2_pt", 100, 0, 500) ;
   TH1F h_dparton2_eta ("h_dparton2_eta", "h_dparton2_eta", 100, -5, 5) ;
   TH1F h_dparton3_pt ("h_dparton3_pt", "h_dparton3_pt", 100, 0, 500) ;
   TH1F h_dparton3_eta ("h_dparton3_eta", "h_dparton3_eta", 100, -5, 5) ;
   TH1F h_dparton4_pt ("h_dparton4_pt", "h_dparton4_pt", 100, 0, 500) ;
   TH1F h_dparton4_eta ("h_dparton4_eta", "h_dparton4_eta", 100, -5, 5) ;
*/
...
//	genjets_v.clear();
	...
	if(genjets_v.size()<4)
	{}
	else// if (genjets_v.size()==4)
	{
       // cout<<"  genjet size "<<genjets.size()<<endl;
	for (int i=0;i<genjets.size();++i){
	//cout<<"  genjet pt   "<<genjets.at(i).Pt()<<endl;
	}
       // cout<<" matched jet size "<<genjets_v.size()<<endl;
        for (int i=0;i<genjets_v.size();++i){
       // cout<<" matched jets pt   "<<genjets_v.at(i).Pt()<<endl;
        }

        h_mparton1_pt.Fill (genjets_v.at (0).Pt());
//	h_mparton1_pt.Divide(&h_parton1_pt);
	//h_dparton1_pt.Fill((h_mparton1_pt.GetEntries())/(h_parton1_pt.GetEntries()));
        h_mparton1_eta.Fill (genjets_v.at (0).Eta());
//        h_mparton1_eta.Divide(&h_parton1_eta);

        h_mparton2_pt.Fill (genjets_v.at (1).Pt());
//        h_mparton2_pt.Divide(&h_parton2_pt);

        h_mparton2_eta.Fill (genjets_v.at (1).Eta());
//        h_mparton2_eta.Divide(&h_parton2_eta);

        h_mparton3_pt.Fill (genjets_v.at (2).Pt());
//        h_mparton3_pt.Divide(&h_parton3_pt);

        h_mparton3_eta.Fill (genjets_v.at (2).Eta());
//        h_mparton3_eta.Divide(&h_parton3_eta);
...
//      float x = h_mparton1_pt.GetEntries()/h_parton1_pt.GetEntries() ;
  //    cout<<x<<endl;
  ...
  h_dparton1_pt.Write ();
/*  h_dparton1_eta.Write ();
  h_dparton2_pt.Write ();
  h_dparton2_eta.Write ();
  h_dparton3_pt.Write ();
  h_dparton3_eta.Write ();
  h_dparton4_pt.Write ();
  h_dparton4_eta.Write ();
