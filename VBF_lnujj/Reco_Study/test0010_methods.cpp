...
//        vector<int> recoUsed (firstN.size (), 0) ;   //PG vector of flags to know if a jet is already used
        vector<int> recoUsed (jets.size (), 0) ;   //PG vector of flags to know if a jet is already used
        vector<int> genUsed (genjets.size (), 0) ; //PG vector of flags to know if a jet is already used
...
//	genjets_v.clear();
   ...
   float vbf_jj_e =-999,   vbf_jj_pt =-999,   vbf_jj_eta=-999,  vbf_jj_phi =-999;   
   ...
   float vbf_wjj_e =-999,   vbf_wjj_pt =-999,   vbf_wjj_eta=-999,  vbf_wjj_phi =-999;   
   ...
   float vbf_lvjj_e =-999,   vbf_lvjj_pt =-999,   vbf_lvjj_eta=-999,  vbf_lvjj_phi =-999, vbf_lvjj_m =-999 ;
...
// 	 float jess 1.0; 
         ...
         for ( size_t i=0; i < jets.size(); ++i) 
         {
            ...
            j_p.SetPxPyPzE (
               ...
               //i_p.SetPtEtaPhiE( jets.at(i).Pt(), jets.at(i).Eta(), jets.at(i).Phi(), jets.at(i).E() );
               //j_p.SetPtEtaPhiE( jets.at(j).Pt(), jets.at(j).Eta(), jets.at(j).Phi(), jets.at(j).E() );
		...
		//cout<<"  "<<sqrt ((i_p+j_p).M2())<<endl;
		  ...
		  vbf_bjp = j_p;                  
                 ...
                 // vbf_bj_m      = (j_p).M();
                  ...
                  vbf_jj_deta   = (jets.at(i).Eta()-jets.at(j).Eta());
                  vbf_jj_dphi   = (jets.at(i).Phi()-jets.at(j).Phi());
...
//	}
	...
	// take two leading jets as W jets other than previously chosen as tag jets
	// method1 B
...
//	{            // 5.  Find two vbf jets and two W jets
	//m1_vbf_event = 1;
	...
	// vbf_aj_id = tag_i_id; vbf_bj_id = tag_j_id; vbf_waj_id = wjj_a_id; vbf_wbj_id = wjj_b_id;
        ...
        if (tag_i_id!=-1&&tag_j_id!=-1&&wjj_a_id!=-1&&wjj_b_id!=-1 && vbf_lvjj_m>0)
        {            // 5.  Find two vbf jets and two W jets
                                 cout<<" four body mass "<<vbf_lvjj_m<<endl;
