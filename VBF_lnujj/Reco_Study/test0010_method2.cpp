//c++ -o test0010_method2 `root-config --glibs --cflags` -lm test0010_method2.cpp
// c++ -o read_007 `root-config --glibs --cflags` -lm read_007.cpp
//  /home/ajay/Work/LPC/Morion13/VBF/RD_mu_TTbar_CMSSW532.root

/*
 method 2 tag jets from complete eta region defalt selection, from remaining two jets more similar to W jets as Wjets
*/
...
// 	 float jess 1.0; 
	...
	for ( int k=0; k < (int) jets.size(); ++k) // loop over jets
        ...
        if (fabs(jets.at(k).Eta()) > 4.7) continue;
		  //if (jets.at(k).Pt() < Jpt) continue;    //cancel events with jet pt<25
		  if ( k != tag_i_id && k!= tag_j_id)
			{ 
			lorentzVector k_p; 
        	   	k_p.SetPxPyPzE (
		   ...
		   for ( int m=k+1; m < (int) jets.size(); ++m) // loop over  jets
		    {
		        if (fabs(jets.at(m).Eta()) > 4.7) continue;
		    //if (jets.at[m].Pt() < Jpt) continue;   //cancel events with jet pt<25
                    if ( m != tag_i_id && m!= tag_j_id) 
			{
			lorentzVector m_p;
                       m_p.SetPxPyPzE (
	...
	} //loop to insure there are allready two tag jets in the event
