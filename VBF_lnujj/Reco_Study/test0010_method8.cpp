...
//c++ -o test0010_method8 `root-config --glibs --cflags` -lm test0010_method8.cpp
/*
fourth method
A) consider only jets with |eta|<2.5, chose the 2 with highest PT
as the ones from W
...
// 	 float jess 1.0; 
	...
	{ 
	//if (fabs(jets.at(k).Eta()) > 2.5) continue;    //choose  leading jets from central region  as W jets 
        if (fabs(jets.at(k).Eta()) > 4.7) continue;    //choose  leading jets from central region  as W jets 
	if (wjj_ajp.Pt()!=0 && wjj_bjp.Pt()==0 ) 
	{
	int Bj = k;  
           wjj_bjp.SetPxPyPzE (
...
//	   { // Find two vbf jets and two W jets
