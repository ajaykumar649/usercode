//c++ -o test0010_method1 `root-config --glibs --cflags` -lm test0010_method1.cpp
// c++ -o read_007 `root-config --glibs --cflags` -lm read_007.cpp
//  .read_007 /home/ajay/Work/LPC/Morion13/VBF/RD_mu_HWWMH300_CMSSW532_private.root

/*
method1 tag jets from whole eta region and from remaining two leading  jets in whole eta region as W jets 
*/
...
//  TString inputFile = argv[1] ;
   ...
   gROOT->Reset();
 TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/afs/cern.ch/work/a/ajkumar/RECOStudy/CMSSW_5_3_2_patch4/src/newFNALntuples/RD_GenJets/RD_mu_HWWMH300_CMSSW532_private.root");
// TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("root://cmssrv32.fnal.gov//eos/uscms/store/user/lnujj/Moriond2013/RD_GenJets/RD_mu_HWWMH300_CMSSW532_private.root");
// TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("inputFile");

   if (!f) {
     f = new TFile("/afs/cern.ch/work/a/ajkumar/RECOStudy/CMSSW_5_3_2_patch4/src/newFNALntuples/RD_GenJets/RD_mu_HWWMH300_CMSSW532_private.root");
//     f = new TFile("root://cmssrv32.fnal.gov//eos/uscms/store/user/lnujj/Moriond2013/RD_GenJets/RD_mu_HWWMH300_CMSSW532_private.root");
...
//  TBranch        *b_GroomedJet_CA[6]_tau2tau1;   //!
  ...
  WJet->SetBranchAddress ("W_H_e_gen",    &W_H_e_gen,    &b_W_H_e_gen   ) ;
   ...
   WJet->SetBranchAddress("W_muon_et", &W_muon_et, &b_W_muon_et);
   WJet->SetBranchAddress("W_muon_eta", &W_muon_eta, &b_W_muon_eta);
   WJet->SetBranchAddress("W_muon_theta", &W_muon_theta, &b_W_muon_theta);
   WJet->SetBranchAddress("W_muon_phi", &W_muon_phi, &b_W_muon_phi);
...
// 	 float jess 1.0; 
	...
	int Bj = k;  
           wjj_bjp.SetPxPyPzE (
                hep_to_polar_x (jets.at(Bj).Pt(), jets.at(Bj).Eta(), jets.at(Bj).Phi()),
                hep_to_polar_y (jets.at(Bj).Pt(), jets.at(Bj).Eta(), jets.at(Bj).Phi()),
                hep_to_polar_z (jets.at(Bj).Pt(), jets.at(Bj).Eta(), jets.at(Bj).Phi()),
                jets.at(Bj).E()
              );
         wjj_b_id=Bj;                                                                                       
	}
	...
	int Aj = k;  
           wjj_ajp.SetPxPyPzE (
                hep_to_polar_x (jets.at(Aj).Pt(), jets.at(Aj).Eta(), jets.at(Aj).Phi()),
                hep_to_polar_y (jets.at(Aj).Pt(), jets.at(Aj).Eta(), jets.at(Aj).Phi()),
                hep_to_polar_z (jets.at(Aj).Pt(), jets.at(Aj).Eta(), jets.at(Aj).Phi()),
                jets.at(Aj).E()
              );
         wjj_a_id=Aj;                                                                                       
	}
...
/*                lepton.SetPxPyPzE (
                hep_to_polar_x (W_muon_pt,W_muon_eta,W_muon_phi),
                hep_to_polar_y (W_muon_pt,W_muon_eta,W_muon_phi),
                hep_to_polar_z (W_muon_pt,W_muon_eta,W_muon_phi),
                W_muon_et
                );
*/
...
//	nutrino.SetPxPyPzM ( event_met_pfmet * cos(event_met_pfmetPhi), event_met_pfmet * sin(event_met_pfmetPhi), 
		nutrino.SetPxPyPzE (
...
//	if (tag_i_id!=-1&&tag_j_id!=-1&&wjj_a_id!=-1&&wjj_b_id!=-1 )
//	{        
...
//		if(vbf_lvjj_m
//cout<<" lep   "<<lepton.Pt()<<"  nu    "<<nutrino.Pt()<<" aj   "<<wjj_ajp.Pt()<<"  bj  "<<wjj_bjp.Pt()<<"  lnujj  "<<vbf_lvjj_m<<endl;	
//cout<<" lep   "<<lepton.Eta()<<"  nu    "<<nutrino.Eta()<<" aj   "<<wjj_ajp.Eta()<<"  bj  "<<wjj_bjp.Eta()<<"  lnujj  "<<vbf_lvjj_m<<endl; 
//cout<<"   "<<tag_i_id<<"    "<<tag_j_id<<"    "<<wjj_a_id<<"    "<<wjj_b_id<<"   "<<vbf_lvjj_m<<"   "<<(lepton+nutrino+wjj_ajp+wjj_bjp).M2()<<endl;

//				cout<<" four body mass "<<vbf_lvjj_m<<endl;
		           // vbf_lvjj_y      = (lepton+nutrino+wjj_ajp+wjj_bjp).Rapidity();
        if (tag_i_id!=-1&&tag_j_id!=-1&&wjj_a_id!=-1&&wjj_b_id!=-1 && vbf_lvjj_m>0 && vbf_lvj_m >30. && vbf_wjj_m >30.)
{			//cout<<ie<<"  "<<tag_i_id<<"  "<<tag_j_id<<"   "<<wjj_a_id<<"  "<<wjj_b_id<<"   "<<vbf_waj_eta<<endl;
cout<<"lnujj_m   "<<vbf_lvjj_m<<endl;
...
//method 1 ends
  ...
  TString outFileName = "VBF300_output_" ;
