...
//c++ -o test0010_method7 `root-config --glibs --cflags` -lm test0010_method7.cpp
/*
third method
        ...
        for ( int k=0; k < (int) jets.size(); ++k) // loop over jets
                {
                 // if (fabs(jets.at(k).Eta()) > 2.5) continue;    //choose w jets from central region closest to W 
                  if (fabs(jets.at(k).Eta()) > 4.7) continue;    //choose w jets from central region closest to W 
                        lorentzVector k_p;
                        k_p.SetPxPyPzE (
                   ...
                   for ( int m=k+1; m < (int) jets.size(); ++m) // loop over  jets
                    ...
                    if (fabs(jets.at(m).Eta()) > 4.7) continue;   //cancel events with jet pt<25
                        lorentzVector m_p;
                       m_p.SetPxPyPzE (
